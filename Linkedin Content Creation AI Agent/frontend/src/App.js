...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchToneStyles = async () => {
      try {
        ...
        setError('Failed to load tone styles');
      }
    };
  ...
  // Move handleCustomCreate here, after useEffect
  const handleCustomCreate = async (data) => {
    try {
      const endpoint = customType === 'template' ? '/templates/custom' : '/tones/custom';
      await axios.post(`${API_URL}${endpoint}`, data);
      // Define these functions inside the handler
      ...
      setError('Failed to create custom template/tone');
    }
  };

    // Add in the form section after template selection
<>
  <Grid item xs={12}>
    <FormControl fullWidth sx={{ mb: 2 }}>
      <InputLabel>Select Tone Style</InputLabel>
      <Select
        value={toneStyle}
      ...
      >
        ...
        {Object.entries(toneStyles).map(([id, tone]) => (
  ...
  // Remove the standalone JSX fragment and merge it into the main return statement
  // Remove the second return statement
  // Move the stats array before the first return statement
                  ...
                  >
                    <form onSubmit={handleSubmit}>
                      <Grid container spacing={3}>
                        ...
                        {/* Add Custom Template/Tone Buttons */}
                        <Grid item xs={12}>
